...
export default function CustomersList({
  ...
  coupons.forEach((c) => {
    if (!couponsByEmail.has(c.customer_email)) {
      couponsByEmail.set(c.customer_email, []);
    }
    couponsByEmail.get(c.customer_email)!.push(c);
  });

  // ---- Stats ----
  const uniqueEmails = new Set(coupons.map((c) => c.customer_email));
  ...
  function renderCouponCard(
    ...
    const sub = submissionMap.get(coupon.submission_id);
    const emailHistory = couponsByEmail.get(coupon.customer_email) || [];
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-0.5 flex items-center gap-2 text-sm text-gray-500">
              <span className="truncate max-w-[180px]">{coupon.customer_email}</span>
          ...
          {isExpanded && !isExpiredVariant && (
            ...
            >
              <div className="border-t border-gray-100 px-5 pb-5 pt-4">
                      ...
                      <div className="space-y-1.5">
                        ...
                        {coupon.email_sent_at && (
                          <div className="flex items-center gap-2 text-xs text-gray-600">
                            ...
                            Coupon email sent {formatDateTime(coupon.email_sent_at)}
                          </div>
                        )}
                        {coupon.used_at && (
                          <div className="flex items-center gap-2 text-xs text-gray-600">
                ...
                {/* Coupon history for this email */}
                {emailHistory.length > 1 && (
                  <div className="mt-4 rounded-xl border border-gray-100 bg-white p-4">
                    <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-2">
                      Coupon History ({emailHistory.length} total)
                    </p>
